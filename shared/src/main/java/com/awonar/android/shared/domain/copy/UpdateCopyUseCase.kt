...
class UpdateCopyUseCase @Inject constructor(
    private val repository: SocialTradeRepository,
    @IoDispatcher dispatcher: CoroutineDispatcher,
) : FlowUseCase<UpdateCopy, Copier?>(dispatcher) {
    override fun execute(parameters: UpdateCopy): Flow<Result<Copier?>> =
        repository.updateCopy(parameters)
}